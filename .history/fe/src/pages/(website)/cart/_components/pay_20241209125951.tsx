...
const generateHMAC = async (secretKey: string, signData: string) => {
...
const CheckoutBoxRight = ({ totalPrice, totalQuantity }: any) => {
  ...
  const handlePayment = (method: string) => {
    ...
    if (method === "vnpay") {
      ...
      // window.location.href = `http://localhost:8080/carts/create-pay/vnpay?amount=${}&ref=${game.game_id}`;
    } else if (method === "momo") {
      ...
      <div className="mt-6 space-y-3">
        ...
        {totalPrice > 0 && (
          <>
            ...
            >
              <span className="mr-2">📱</span> Thanh toán qr VnPay
            </button>
          </>
        )}
      </div>
