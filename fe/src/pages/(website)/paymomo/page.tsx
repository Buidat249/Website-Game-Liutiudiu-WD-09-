...
import CartItemMomo from "./_components/itemvnpay";
import CheckoutSummaryMomo from "./_components/vnpayconfirm";
    ...
    <div className="flex flex-col lg:flex-row bg-gray-100 p-6 mx-auto w-[1048px] space-y-6 lg:space-y-0 lg:space-x-6">
      ...
      <div className="flex-1 bg-white p-6 rounded-lg shadow-md">
        ...
        {selectedItems.map((item) => (
          <CartItemMomo key={item.game_id} game={item} quantity={item.quantity} />
        ))}
      </div>
      ...
      <div className="w-full lg:w-1/3 bg-white p-6 rounded-lg shadow-md">
        ...
        <CheckoutSummaryMomo games={selectedItems} />
      </div>
    </div>
  );
};
