...
// GET /tintucs/category/:categorynew_id
export const getTintucsByCategory = async (req, res) => {
  try {
    const { categorynew_id } = req.params;
    const tintucs = await Tintuc.find({ categorynew_id: categorynew_id });
    if (!tintucs || tintucs.length === 0) {
      return res.status(404).json({
        message: "No Tintucs Found for this Category",
      });
    }
    return res.status(200).json({
      message: "Get Tintucs By Category Done",
      data: tintucs,
    });
  } catch (error) {
    return res.status(500).json({ message: error.message });
...
export const addTintuc = async (req, res) => {
  ...
  try {
    ...
    const lastTintuc = await Tintuc.findOne(
      {},
      {},
      { sort: { tintuc_id: -1 } }
    );
